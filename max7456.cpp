...
//-----------------------------------------------------------------------------
// Implements Max7456::getVideoMode
//-----------------------------------------------------------------------------
byte Max7456::getVideoMode()
{
	return _regVm0.bits.videoSelect;
}

//-----------------------------------------------------------------------------
// Implements Max7456::getLineCount
//-----------------------------------------------------------------------------
byte Max7456::getLineCount()
{
	return getVideoMode() == 0 ? 13 : 16;
}
...
void Max7456::printMax7456Chars(byte chars[], byte size, byte x, byte y, byte blink, byte inv)
{
	...
	{
		uint8_t debugBuff[size + 3];
		debugBuff[0] = x;
		debugBuff[1] = y;
		debugBuff[2] = size;
		for (int i = 0; i < size; i++)
		{
			debugBuff[i + 3] = chars[i];
		}
...
#elif MAX7456_TABLE_COSD == 1
	if (ascii == ' ')
		ascii = 0x00; // remap space
	else if (ascii > 48 && ascii <= 57)
		ascii -= 48; // remap numbers
	else if (ascii == '0')
		ascii = 10; // remap zero
	else if (ascii >= 65 && ascii <= 90)
		ascii -= 54; // remap big letters
	else if (ascii >= 97 && ascii <= 122)
		ascii -= 60; // remap small letters
	else if (ascii == '(')
		ascii = 63; // remap
	else if (ascii == ')')
		ascii = 64; // remap
...
//-----------------------------------------------------------------------------
void Max7456::init(byte iPinCS, byte videoMode)
{
	_pinCS = iPinCS;
...
//-----------------------------------------------------------------------------
void Max7456::activateOSD(bool act, byte videoMode)
{
